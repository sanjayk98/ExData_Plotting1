...
plot(as.POSIXct(paste(power$Date, power$Time), format="%d/%m/%Y %H:%M:%S"),as.numeric(power$Voltage),type="l",col="red",main="Voltage",xlab="Date",ylab="Volt")
...
plot(as.POSIXct(paste(power$Date, power$Time), format="%d/%m/%Y %H:%M:%S"),as.numeric(power$Global_reactive_power),type="l",col="red",main="global reactive power",xlab="Date",ylab="kw")
dev.off()